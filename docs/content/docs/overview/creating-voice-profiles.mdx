...
A **voice profile** is a saved voice you can reuse across generations, stories, and the API. Voicebox profiles come in two flavors that map to two different ways of getting a voice:
...
- **Cloned profiles** export with their reference audio and transcripts, plus supported metadata and optional avatar
...
Reference text is limited to 1,000 characters per sample. A sample endpoint needs
the confirmed transcript; automatic transcription in the UI is an editing aid.
