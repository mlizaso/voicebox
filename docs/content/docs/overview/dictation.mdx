...
Processing runs on the selected backend. A local backend keeps audio on this
machine; a remote connection sends it to that server. Enable dictation and complete
the readiness/permission checks in **Settings → Captures** before using the chord.
New settings leave the global hotkey disabled until you enable it.
...
whatever text field had focus when you started the chord. The clipboard is
saved first and restored only if it has not changed during the paste window, so
newer clipboard contents are preserved.
...
**The target is captured at chord-start.** Native checks validate the captured
target before pasting. If the target is no longer valid, use the saved transcript
in Captures instead of relying on whichever window happens to be focused.
...
What refinement aims to preserve (review the result):
...
| Qwen3 4B | ~2.5 GB | Largest available option; compare results on your content. |

These approximate download sizes describe MLX variants; PyTorch weights differ.
...
Profiles, recordings, history, and generated audio belong to the selected backend.
See [storage and backup](/overview/storage-and-backup) for actual paths and backups,
and [quick start](/overview/quick-start) for the current workflow.

Changing servers during a recording cancels the eventual upload. Native focus/paste
targets are short-lived and validated; a missing target must not paste into another app.
