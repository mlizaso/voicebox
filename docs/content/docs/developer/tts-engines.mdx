...
The registry is one part of an integration. Confirm request schemas, engine/model
validation, profile compatibility, frontend choices, and packaging as well. Preserve
MLX lifecycle and exact-runtime identity checks. The current module map is in
[architecture](/developer/architecture); do not treat an old example below as proof
that routes/services need no review.
...
|-------|---------|---------------|
| `routes/` | HTTP validation and dispatch | Review accepted engines, streaming, and exact routes |
| `services/` | Generation and profile operations | Review compatibility, queueing, and reference handling |
| `backends/` | Engine implementations | `your_engine_backend.py` |
| `utils/` | Shared utilities | As needed |

The model registry centralizes dispatch, but it does not replace request validation, profile compatibility, lifecycle management, or packaging. Review those paths when adding an engine; change them only where the integration requires it.
